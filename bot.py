...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "🔎 Boshqa arizalarni ko'rish uchun \"Arizalarni ko'rish\" tugmasini bosing.\n"
        "🧾 \"Mening e'lonlarim\" bo'limidan topshirgan arizalaringiz holatini kuzatib borishingiz mumkin.",
        reply_markup=main_menu_keyboard(is_admin(message.from_user.id)),
    )
...
async def cmd_vacancies(message: Message, state: FSMContext):
    await state.set_state(BrowseVacancies.region)
    await message.answer("Qaysi hudud bo'yicha arizalarni ko'rmoqchisiz?", reply_markup=regions_keyboard("browseregion"))
